...
from wurm.envs import SingleSnakeEnvironments4
...
if args.env == 'gridworld':
    ...
    env = SingleSnakeEnvironments4(num_envs=args.num_envs, size=args.size, device=args.device,
...
t0 = time()
state = env.reset()
last_p = torch.zeros((args.num_envs,)).float().requires_grad_(False)
for i_step in count(1):
    ...
    state, reward, done, info, new_p = env.step(action,last_p)
    last_p = new_p
